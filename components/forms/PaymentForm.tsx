...
const PaymentForm: React.FC<PaymentFormProps> = ({ invoice, onSave, onCancel, paymentMethods }) => {
    const { balance } = getInvoiceStatusAndBalance(invoice);
    const [payments, setPayments] = useState([{ id: 1, method: paymentMethods[0]?.name || '', amount: balance > 0 ? balance.toFixed(2) : '' }]);
    const [nextId, setNextId] = useState(2);
    const [receiptNumber, setReceiptNumber] = useState('');

    const handlePaymentChange = (id: number, field: 'method' | 'amount', value: any) => {
        setPayments(current => current.map(p => p.id === id ? { ...p, [field]: value } : p));
    };

    const addPayment = () => {
        setPayments(current => [...current, { id: nextId, method: paymentMethods[0]?.name || '', amount: '' }]);
                ...
                <div className="lg:col-span-3 card">
                     ...
                     <div className="card-body space-y-4">
                        {payments.map((p) => (
                            <div key={p.id} className="p-3 bg-slate-800/50 border border-slate-700 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                <div className="md:col-span-2">
                                    ...
                                    <select value={p.method} onChange={(e) => handlePaymentChange(p.id, 'method', e.target.value)} className="form-select">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                         <input type="number" step="0.01" value={p.amount} onChange={(e) => handlePaymentChange(p.id, 'amount', e.target.value)} placeholder="0.00" className="form-input pr-12"/>
                        ...
                        <div className="pt-4 border-t border-slate-700">
                            ...
                            <input id="receiptNumber" type="text" value={receiptNumber} onChange={e => setReceiptNumber(e.target.value)} placeholder="Ex: 2024001" className="form-input"/>
                        </div>
                     </div>
                </div>
            </div>
